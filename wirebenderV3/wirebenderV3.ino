  ...
  if (runState == STATE_READY) {  // wait for START messaage
    ...
    if (Serial.available() > 0) {
      ...
      if (input == "START") {
        ...
        bendStepper.setCurrentPosition(0);
...
//    Serial.println("\t\t\t\t\t\tboth distances to go are zero");
    if (prevFeedDist != 0 || prevBendDist != 0) {
      if (bending == 0) {
        ...
        //djfklajfdlsajfklsajflksajdfklajskdflajkflasjfdlasjfldsafasfd
        bendStepper.moveTo(0);
        bending = 0;
        delay(300);
      }
    }
...
bool initAction() {
  if (queue[currentActionIndex].action != ACTION_NONE) {   
    if (queue[currentActionIndex].action == ACTION_FEED) {
      ...
      bending = 1;
      bendStepper.moveTo(degToStepsBend(queue[currentActionIndex].actionAmount));
